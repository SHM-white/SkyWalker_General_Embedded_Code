...
public:
    ...
    const InterBoardParser::Stats &stats() const { return parser_.stats(); }
    std::uint32_t rejectedFrames() const { return rejected_; }
private:
    void accept(const FrameMeta &,const std::uint8_t *,std::size_t);
    BoardRole role_; InterBoardParser parser_{};
    std::array<robotics::MessageStamp,4> sequences_{};
    robotics::BoardHeartbeat heartbeat_{}; robotics::RemoteChassisControl control_{};
    robotics::ChassisConstraint constraint_{}; robotics::ChassisFeedbackSummary feedback_{};
