...
public:
    struct Stats {
        std::uint32_t valid_frames = 0, crc8_errors = 0, crc16_errors = 0, length_errors = 0, unknown_commands = 0,
                      assembly_timeouts = 0;
    };
...
private:
    ...
    RefereeVersion version_;
    std::array<std::uint8_t, 256> buffer_{};
    std::array<std::uint64_t, 256> receive_ms_{};
    std::size_t used_ = 0;
