...
int main() {
    ...
    LOG_INF(
        "Single suspended module: e enable, space disable, w/s forward/reverse, a/d lateral, q rotate, ! estop, r reset");
    ...
    for (;;) {
        ...
        const auto fresh = [&](const motor::MotorSnapshot &view) {
            ...
            return view.feedback_fresh && std::isfinite(f.position_rad) && std::isfinite(f.velocity_rad_s) &&
                   std::fabs(f.velocity_rad_s) < 40 &&
        ...
        if (healthy && !ready && !group_status.active && !group_status.enable_pending && module_group.ready()) {
            if (stable_since == 0) {
                stable_since = now;
                first_stamp = steer_view.feedback.timestamp_ms;
            }
            else if (now - stable_since >= 30 && steer_view.feedback.timestamp_ms != first_stamp) {
                ...
                if (ret == 0) {
                    ...
                    feedback = {bench::steer_direction * sf.absolute_position_rad,
                                bench::steer_direction * sf.position_rad, bench::steer_direction * sf.velocity_rad_s,
        ...
        if (requested && !estop && ready && healthy && !enable_issued && module_group.ready()) {
        ...
        if (now >= next_log) {
            ...
            LOG_INF(
                "active=%d pending=%d ready=%d feedback=%d err=%d bus=%u target=%.3f angle=%.3f wheel=%.3f drive=%.3f current=%.3f/%.3f",
