...
int main() {
    ...
    GlobalSafetyManager safety({false, 300});
    ...
    for (;;) {
        ...
        if (now >= next_log) {
            next_log = now + 100;
            LOG_INF("rc=%d mode=%u safety=%u reason=%x chassis=%.2f/%.2f/%.2f yaw_rate=%.2f yaw_mode=%u seq=%u",
                    r.online, unsigned(intent.mode), unsigned(decision.state), decision.active_reasons,
                    double(command.chassis.vx_m_s), double(command.chassis.vy_m_s), double(command.chassis.wz_rad_s),
                    double(command.gimbal.yaw_rate_rad_s), unsigned(command.gimbal.mode), command.stamp.sequence);
        }
        k_sleep(K_MSEC(10));
    }
}
