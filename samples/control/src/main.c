...
static int run_feedforward_pid_sample(Vofa *vofa) {
    ...
    if (!float_near(result.output, 5.0f) || !float_near(state.feedback.integral_output, 0.0f) ||
...
static int run_reference_helpers_sample(Vofa *vofa) {
    ...
    ret = control_slew_rate_step(&slew_state, &slew_config, 10.0f, 0.1f, &value, &rate);
    if (ret < 0) {
        return ret;
    }
    ...
    ret = control_shortest_angle_error(3.14159265358979323846f, 0.0f, &angle_error);
    if (ret < 0) {
        return ret;
    }
