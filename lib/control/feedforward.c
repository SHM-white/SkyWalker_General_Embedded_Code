...
static float direction_from_reference(const control_feedforward_config *config, const control_feedforward_reference *reference) {
    ...
    if (fabsf(reference->acceleration_ref) > config->acceleration_epsilon) {
...
int control_feedforward_calculate(const control_feedforward_config *config, const control_feedforward_reference *reference, float *output) {
    ...
    const float direction = direction_from_reference(config, reference);
    ...
    case CONTROL_GRAVITY_SIN:
        gravity = config->k_gravity * sinf(reference->position_ref_rad);
        break;
    case CONTROL_GRAVITY_COS:
        gravity = config->k_gravity * cosf(reference->position_ref_rad);
        break;
    default:
        return -EINVAL;
    }
