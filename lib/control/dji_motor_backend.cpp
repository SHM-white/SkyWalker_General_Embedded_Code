...
int DjiMotorBackend::describe(MotorInfo &info) {
    ...
    if ((caps & motor::CommandCurrent) == 0) return -ENOTSUP;
    info = {EffortUnit::Ampere, descriptor_.configured_current_limit_a, caps,
...
int DjiMotorBackend::prepare() {
    ...
    for (;;) {
        ...
        if (state == motor::State::Ready) return 0;
...
int DjiMotorBackend::write(float effort_a) { return motor::setCurrent(motor_, effort_a); }
int DjiMotorBackend::flush() { return bus_.flush(report_); }

int DjiMotorBackend::stop() {
    if (bus_.state() == motor::dji::BusState::Uninitialized) return 0;
