...
int control_pid_validate(const control_pid_config *config) {
    ...
    if (config->dt_min_s <= 0.0f || config->dt_max_s < config->dt_min_s) {
        return -EINVAL;
    }
    return 0;
}
...
int control_pid_step_core(control_pid_state *state, const control_pid_config *config, const control_pid_input *input,
    ...
    const float effective_error = fabsf(error) <= config->deadband ? 0.0f : error;
    ...
    if (!first_step) {
        measurement_rate = (input->measurement - next.previous_measurement) / input->dt_s;
    ...
    if (config->derivative_tau_s > 0.0f) {
        const float denominator = config->derivative_tau_s + input->dt_s;
        const float alpha = input->dt_s / denominator;
        filtered_rate = next.filtered_measurement_rate + alpha * (measurement_rate - next.filtered_measurement_rate);
    }
    ...
    const float integral_delta = config->ki * effective_error * input->dt_s;
    const float integral_unclamped = next.integral_output + integral_delta;
    if (!isfinite(integral_delta) || !isfinite(integral_unclamped)) {
        return -ERANGE;
    }

    const float integral_candidate = clampf(integral_unclamped, config->integral_min, config->integral_max);

    const float candidate_feedback = p + integral_candidate + d;
    const float candidate_total = candidate_feedback + additive_output;
    if (!isfinite(candidate_feedback) || !isfinite(candidate_total)) {
        return -ERANGE;
    }

    const bool pushes_high_saturation = candidate_total > config->output_max && effective_error > 0.0f;
    const bool pushes_low_saturation = candidate_total < config->output_min && effective_error < 0.0f;

    if (!input->freeze_integrator && !pushes_high_saturation && !pushes_low_saturation) {
    ...
    local.d = d;
    local.feedback_unsaturated = local.p + local.i + local.d;
    local.total_unsaturated = local.feedback_unsaturated + additive_output;

    if (!isfinite(local.feedback_unsaturated) || !isfinite(local.total_unsaturated)) {
        return -ERANGE;
    }

    local.output = clampf(local.total_unsaturated, config->output_min, config->output_max);
    local.saturated = local.total_unsaturated < config->output_min || local.total_unsaturated > config->output_max;
