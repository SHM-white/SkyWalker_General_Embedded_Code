    ...
    const control_motor_velocity_config *config)
{
    ...
    if (!isfinite(config->measurement_filter_tau_s) ||
        !isfinite(config->soft_deadband_rad_s) ||
        !isfinite(config->requested_velocity_abs_max_rad_s) ||
        !isfinite(config->current_abs_max_a)) {
        return -EINVAL;
    }
    if (config->measurement_filter_tau_s < 0.0f ||
        config->soft_deadband_rad_s < 0.0f ||
        config->requested_velocity_abs_max_rad_s <= 0.0f ||
        config->current_abs_max_a < 0.0f) {
        return -EINVAL;
    }
    return 0;
}
    ...
    float initial_reference_rad_s)
{
    if (state == NULL ||
        ...
        !isfinite(initial_reference_rad_s)) {
        return -EINVAL;
    }

    control_motor_velocity_state next = {0};
    int ret = control_feedforward_pid_reset(
        &next.regulator, measured_velocity_rad_s);
    if (ret < 0) {
        return ret;
    }
    ret = control_slew_rate_reset(
        &next.reference_slew, initial_reference_rad_s);
    if (ret < 0) {
        return ret;
    }
    ...
    control_motor_velocity_output *output)
{
    ...
    if (!isfinite(input->requested_velocity_rad_s) ||
        !isfinite(input->measured_velocity_rad_s) ||
        !isfinite(input->position_reference_rad) ||
        !isfinite(input->dt_s)) {
        return -EINVAL;
    }
    if (fabsf(input->requested_velocity_rad_s) >
        config->requested_velocity_abs_max_rad_s) {
        return -ERANGE;
    }
    ...
    const float filter_denominator =
        config->measurement_filter_tau_s + input->dt_s;
    ...
    const float filtered_velocity =
        next.filtered_velocity_rad_s +
        alpha * (input->measured_velocity_rad_s -
                 next.filtered_velocity_rad_s);
    ...
    ret = control_slew_rate_step(
        ...
        input->requested_velocity_rad_s,
        input->dt_s,
        &local.velocity_reference_rad_s,
        &local.acceleration_reference_rad_s2);
    if (ret < 0) {
        return ret;
    }

    local.velocity_error_rad_s =
        local.velocity_reference_rad_s - filtered_velocity;
    const float effective_error = apply_soft_deadband(
        local.velocity_error_rad_s, config->soft_deadband_rad_s);
    const float regulator_measurement =
        local.velocity_reference_rad_s - effective_error;
    if (!isfinite(local.velocity_error_rad_s) ||
        !isfinite(effective_error) ||
        !isfinite(regulator_measurement)) {
        return -ERANGE;
    }
    ...
    local.current_command_a = clampf(
        local.regulator.output,
        -config->current_abs_max_a,
        config->current_abs_max_a);
