...
const DmConfig *configOf(const struct device *dev)
{
    return dev == nullptr
        ? nullptr
        : static_cast<const DmConfig *>(dev->config);
}
...
std::uint32_t getCapabilities(const struct device *dev)
{
    ...
    std::uint32_t capabilities =
        FeedbackPosition | FeedbackVelocity | FeedbackTorque |
        FeedbackTemperature;
...
State getStateImpl(const struct device *dev)
{
    ...
    const bool fault = data->fault_latched ||
                       isFaultStatus(data->drive_status);
...
int stageFrame(const struct device *dev,
...
int dmMotorInit(const struct device *dev)
{
    DmData *data = dataOf(dev);
    const DmConfig *cfg = configOf(dev);
    if (dev == nullptr || data == nullptr || cfg == nullptr ||
        cfg->can == nullptr) {
        return -EINVAL;
    }
    ...
    if (cfg->motor_id == 0u || cfg->motor_id > 15u ||
        cfg->master_id > CAN_STD_ID_MASK) {
        return -ERANGE;
    }
    if (cfg->mode != ControlMode::Mit &&
        cfg->mode != ControlMode::PositionVelocity &&
    ...
    limits.position_max_rad =
        static_cast<float>(cfg->p_max_millirad) / 1000.0f;
    limits.velocity_max_rad_s =
        static_cast<float>(cfg->v_max_millirad_s) / 1000.0f;
    limits.torque_max_nm =
        static_cast<float>(cfg->t_max_millinewton_meter) / 1000.0f;
    const float torque_limit =
        static_cast<float>(cfg->torque_limit_millinewton_meter) /
        1000.0f;
    if (!std::isfinite(limits.position_max_rad) ||
        ...
        !std::isfinite(torque_limit) || torque_limit <= 0.0f ||
        torque_limit > limits.torque_max_nm) {
        return -ERANGE;
    }

    std::uint16_t command_id = 0u;
    const int id_ret = controlFrameId(
        cfg->mode,
        static_cast<std::uint16_t>(cfg->motor_id),
        command_id);
    if (id_ret < 0) {
        return id_ret;
    }
                   ...
                   std::uint64_t now_ms)
{
    ...
    feedback.temperature_c =
        static_cast<float>(decoded.raw.rotor_temperature_c);
    feedback.valid = FeedbackPosition | FeedbackVelocity |
                     FeedbackTorque | FeedbackTemperature;
...
int buildNeutralFrame(const struct device *dev,
                      struct can_frame &out)
{
    ...
    case ControlMode::Mit:
        return buildMitFrame(
            static_cast<std::uint16_t>(cfg->motor_id),
            data->limits,
            MitCommand{},
            out);
    case ControlMode::PositionVelocity:
        return buildPositionVelocityFrame(
            static_cast<std::uint16_t>(cfg->motor_id),
            current_position,
            0.0f,
            out);
    case ControlMode::Velocity:
        return buildVelocityFrame(
            static_cast<std::uint16_t>(cfg->motor_id),
            0.0f,
            out);
    }
...
bool recoveryReady(const struct device *dev,
                   std::uint64_t recovery_started_ms)
{
    DmData *data = dataOf(dev);
    ...
    const std::uint64_t now_ms =
        static_cast<std::uint64_t>(k_uptime_get());
    const k_spinlock_key_t key = k_spin_lock(&data->lock);
    const bool ready = feedbackFreshLocked(*data, now_ms) &&
                       data->last_rx_ms > recovery_started_ms &&
                    ...
                    CommandSnapshot &out)
{
    ...
    if (!data->armed || data->fault_latched ||
        isFaultStatus(data->drive_status)) {
        ret = -EACCES;
    } else if (!data->command_valid ||
               data->active_epoch != expected_epoch ||
        ...
        ret = -EHOSTDOWN;
               ...
               now_ms - data->command_stamp_ms >
                   CONFIG_SKYWALKER_DM_COMMAND_TIMEOUT_MS) {
        ret = -ESTALE;
    } else {
...
int describe(const struct device *dev, Descriptor &out)
{
    ...
    const internal::DmConfig *cfg =
        static_cast<const internal::DmConfig *>(dev->config);
    internal::DmData *data =
        static_cast<internal::DmData *>(dev->data);
    ...
    const int ret = controlFrameId(next.mode, next.motor_id,
                                   next.control_id);
    if (ret < 0) {
        return ret;
    }
                  ...
                  const MitCommand &command)
{
    ...
    if (!std::isfinite(command.torque_ff_nm) ||
        std::fabs(command.torque_ff_nm) > data->torque_limit_nm) {
        return -ERANGE;
    }

    struct can_frame frame{};
    const int ret = buildMitFrame(
        static_cast<std::uint16_t>(cfg->motor_id),
        data->limits,
        command,
        frame);
                        ...
                        float velocity_limit_rad_s)
{
    ...
    if (!std::isfinite(position_rad) ||
        !std::isfinite(velocity_limit_rad_s)) {
        return -EINVAL;
    }
    if (std::fabs(position_rad) > data->limits.position_max_rad ||
    ...
    struct can_frame frame{};
    const int ret = buildPositionVelocityFrame(
        static_cast<std::uint16_t>(cfg->motor_id),
        position_rad,
...
int setVelocity(const struct device *dev, float velocity_rad_s)
{
    ...
    struct can_frame frame{};
    const int ret = buildVelocityFrame(
        static_cast<std::uint16_t>(cfg->motor_id),
        velocity_rad_s,
        frame);
    if (ret < 0) {
