...
int floatToUint(float value, float minimum, float maximum, unsigned int bits, std::uint32_t &out) {
    ...
    const float scaled = (value - minimum) * static_cast<float>(full_scale) / (maximum - minimum);
...
float uintToFloat(std::uint32_t value, float minimum, float maximum, unsigned int bits) {
    const std::uint32_t full_scale = (1u << bits) - 1u;
    return static_cast<float>(value) * (maximum - minimum) / static_cast<float>(full_scale) + minimum;
}

void writeLeFloat(float value, std::uint8_t *out) {
    static_assert(sizeof(float) == sizeof(std::uint32_t), "Damiao protocol requires 32-bit IEEE-754 float");
...
DriveStatus decodeStatus(std::uint8_t raw) {
    switch (raw) {
    case 0x0u:
        return DriveStatus::Disabled;
    case 0x1u:
        return DriveStatus::Enabled;
    case 0x8u:
        return DriveStatus::OverVoltage;
    case 0x9u:
        return DriveStatus::UnderVoltage;
    case 0xAu:
        return DriveStatus::OverCurrent;
    case 0xBu:
        return DriveStatus::MosOverTemperature;
    case 0xCu:
        return DriveStatus::MotorOverTemperature;
    case 0xDu:
        return DriveStatus::CommunicationLost;
    case 0xEu:
        return DriveStatus::Overload;
    default:
...
bool isFaultStatus(DriveStatus status) {
    return status != DriveStatus::Disabled && status != DriveStatus::Enabled;
}
...
int buildMitFrame(std::uint16_t motor_id, const Limits &limits, const MitCommand &command, struct can_frame &out) {
    ...
    ret = floatToUint(command.position_rad, -limits.position_max_rad, limits.position_max_rad, 16u, position);
    if (ret < 0)
        return ret;
    ret = floatToUint(command.velocity_rad_s, -limits.velocity_max_rad_s, limits.velocity_max_rad_s, 12u, velocity);
    if (ret < 0)
        return ret;
    ...
    if (ret < 0)
        return ret;
    ret = floatToUint(command.torque_ff_nm, -limits.torque_max_nm, limits.torque_max_nm, 12u, torque);
    if (ret < 0)
        return ret;
    ...
    next.data[3] = static_cast<std::uint8_t>(((velocity & 0x0Fu) << 4) | (kp >> 8));
    next.data[4] = static_cast<std::uint8_t>(kp);
    next.data[5] = static_cast<std::uint8_t>(kd >> 4);
    next.data[6] = static_cast<std::uint8_t>(((kd & 0x0Fu) << 4) | (torque >> 8));
...
int buildPositionVelocityFrame(std::uint16_t motor_id, float position_rad, float velocity_rad_s,
    ...
    std::uint16_t command_id = 0u;
    const int ret = controlFrameId(ControlMode::PositionVelocity, motor_id, command_id);
    if (ret < 0) {
        return ret;
    }
...
int buildVelocityFrame(std::uint16_t motor_id, float velocity_rad_s, struct can_frame &out) {
    ...
    std::uint16_t command_id = 0u;
    const int ret = controlFrameId(ControlMode::Velocity, motor_id, command_id);
    if (ret < 0) {
        return ret;
    }
