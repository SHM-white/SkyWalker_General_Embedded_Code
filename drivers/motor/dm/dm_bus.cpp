...
int Bus::attach(const struct device *motor)
{
    ...
    for (std::size_t i = 0; i < motor_count_; ++i) {
        ...
        if (descriptors_[i].master_id == descriptor.master_id &&
            descriptors_[i].motor_id == descriptor.motor_id) {
            return -EADDRINUSE;
        }
    ...
    if (needs_route &&
        route_count_ >= CONFIG_SKYWALKER_DM_MAX_MASTER_IDS_PER_BUS) {
        return -ENOSPC;
    }
...
void Bus::rxCallback(const struct device *,
...
void Bus::dispatchFeedback(const struct can_frame &frame)
{
    ...
    const k_spinlock_key_t key = k_spin_lock(&route_lock_);
    for (std::size_t i = 0; i < motor_count_; ++i) {
        if (descriptors_[i].master_id == frame.id &&
            descriptors_[i].motor_id == motor_id) {
            target = motors_[i];
            break;
        }
    }
...
int Bus::sendFrame(const struct can_frame &frame,
...
int Bus::disableAll(TxReport &report, bool latch_fault)
{
    ...
    const std::size_t expected =
        static_cast<std::size_t>(report.frames_expected) + motor_count_;
    report.frames_expected = static_cast<std::uint8_t>(
        expected > std::numeric_limits<std::uint8_t>::max()
    ...
    for (std::size_t i = 0; i < motor_count_; ++i) {
        struct can_frame frame{};
        int ret = buildSpecialFrame(
            descriptors_[i].mode,
            descriptors_[i].motor_id,
            SpecialCommand::Disable,
            frame);
        if (ret == 0) {
...
{
    const int original_error = report.tx_error < 0
        ? report.tx_error
...
int Bus::arm(TxReport &report)
{
    ...
    struct can_frame
        neutral_frames[CONFIG_SKYWALKER_DM_MAX_MOTORS_PER_BUS]{};
    for (std::size_t i = 0; i < motor_count_; ++i) {
        int ret = internal::preflightArm(motors_[i]);
        if (ret == 0) {
            ret = internal::buildNeutralFrame(
                motors_[i],
                neutral_frames[i]);
        }
        if (ret < 0) {
    ...
    report.frames_expected =
        static_cast<std::uint8_t>(motor_count_ * 2u);
    ...
    const std::uint64_t now_ms =
        static_cast<std::uint64_t>(k_uptime_get());
    for (std::size_t i = 0; i < motor_count_; ++i) {
        const int ret = internal::armMotor(
            motors_[i],
            lifecycle_epoch_,
            now_ms == 0u ? 1u : now_ms,
            neutral_frames[i]);
...
int Bus::flush(TxReport &report)
{
    ...
    internal::CommandSnapshot
        snapshots[CONFIG_SKYWALKER_DM_MAX_MOTORS_PER_BUS]{};
    const std::uint64_t now_ms =
        static_cast<std::uint64_t>(k_uptime_get());
    for (std::size_t i = 0; i < motor_count_; ++i) {
        const int ret = internal::snapshotCommand(
            motors_[i],
            lifecycle_epoch_,
            now_ms,
            snapshots[i]);
    ...
    if (recovery_started_ms_ == 0u) {
        ...
        for (std::size_t i = 0; i < motor_count_; ++i) {
            ...
            int ret = buildSpecialFrame(
                descriptors_[i].mode,
                descriptors_[i].motor_id,
                SpecialCommand::ClearError,
                clear_frame);
            if (ret == 0) {
                ret = buildSpecialFrame(
                    descriptors_[i].mode,
                    descriptors_[i].motor_id,
                    SpecialCommand::Disable,
                    disable_frame);
...
int Bus::savePositionZero(const struct device *motor, TxReport &report)
{
    ...
    struct can_frame frame{};
    const int build_ret = buildSpecialFrame(
        descriptors_[motor_index].mode,
        descriptors_[motor_index].motor_id,
        SpecialCommand::SaveZero,
        frame);
    if (build_ret < 0) {
